...
export async function rejectCreator(userId) {
  try {
    await api.post(`/admin/users/${userId}/reject-creator`);
    return true;
  } catch (error) {
    throw normalizeAxiosError(error);
  }
...
export default { getPendingGroups, approveGroup, rejectGroup, getAllUsers, approveCreator, rejectCreator };
