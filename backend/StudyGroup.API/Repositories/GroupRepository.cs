...
public class GroupRepository : IGroupRepository
{
    ...
    public async Task<List<Group>> GetAllApprovedAsync(string? subject, string? search, string? location, string? meetingTime, string? creatorName = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(creatorName))
            query = query.Where(g => g.Owner.FullName.Contains(creatorName));
    ...
    public async Task<Group?> GetByIdAsync(Guid id)
    {
        return await _db.Groups
            ...
            .Include(g => g.DiscussionMessages)
