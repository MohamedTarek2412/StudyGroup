...
public class JoinRequestService : IJoinRequestService
{
    ...
    public async Task<JoinRequestListItemDto> CreateAsync(Guid groupId, Guid userId)
    {
        ...
        // Notify the group owner about the new join request
        await _notifications.CreateNotificationAsync(
            group.OwnerId,
            $"A new student has requested to join your group \"{group.Name}\".");
    ...
    private async Task UpdateStatus(Guid requestId, Guid requesterId, JoinRequestStatus status)
    {
        ...
        var statusText = status == JoinRequestStatus.Approved ? "approved" : "rejected";
        await _notifications.CreateNotificationAsync(
            jr.UserId,
            $"Your request to join the group \"{jr.Group.Name}\" has been {statusText}.");
    }
