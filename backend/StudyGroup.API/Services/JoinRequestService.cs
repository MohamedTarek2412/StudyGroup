...
public class JoinRequestService : IJoinRequestService
{
    ...
    private async Task UpdateStatus(Guid requestId, Guid requesterId, JoinRequestStatus status)
    {
        ...
        if (status == JoinRequestStatus.Approved && jr.Status != JoinRequestStatus.Approved)
        {
            // MaxMembers is the student cap (same as GroupDetailsDto.MemberCount, which excludes the owner).
            var approvedStudents = jr.Group.JoinRequests.Count(j =>
                j.Status == JoinRequestStatus.Approved);

            if (jr.Group.MaxMembers > 0 && approvedStudents >= jr.Group.MaxMembers)
                throw new InvalidOperationException("This group has reached its maximum number of members.");
        }
