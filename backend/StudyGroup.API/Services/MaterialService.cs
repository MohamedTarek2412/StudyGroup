...
public class MaterialService : IMaterialService
{
        ...
        IWebHostEnvironment env)
    {
        ...
        _uploadRoot = ResolveUploadRoot(config, env);
    }

    private static string ResolveUploadRoot(IConfiguration config, IWebHostEnvironment env)
    {
        var configured = config["FileStorage:UploadPath"];
    ...
    public async Task<MaterialListItemDto> UploadMaterialAsync(Guid groupId, IFormFile file, Guid uploaderId)
    {
        ...
        var uploadsFolder = Path.Combine(_uploadRoot, "Materials", groupId.ToString());
        ...
        membersToNotify.Remove(uploaderId);

        foreach (var memberId in membersToNotify)
        {
            await _notifications.CreateNotificationAsync(memberId, $"New material '{file.FileName}' was uploaded to '{group.Name}'.");
        }

        await _notifications.BroadcastNotificationAsync($"{saved.UploadedBy.FullName} just uploaded a new material to '{group.Name}'!");
    ...
    public async Task DeleteMaterialAsync(Guid materialId, Guid requesterId)
    {
        ...
        await _notifications.BroadcastNotificationAsync($"A material '{material.FileName}' was removed from the group '{group.Name}'.");
    }
    ...
    private static MaterialListItemDto Map(Material m) => new(
        m.Id, m.FileName, m.FilePath, m.FileSizeBytes, m.DownloadCount,
